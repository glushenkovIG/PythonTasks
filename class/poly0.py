...
class Poly():
    ...
    def __add__(self, other):
        ...
        if type(other) in [int, float, Fraction]:
            ...
            min_poly = other.x.copy()
            m = len(poly)
            n = len(min_poly)
            if m < n:
                poly = poly + [0] * (n - m)
            n = min(len(poly), len(min_poly))
            for i in range(n):
                poly[i] += min_poly[i]
        return Poly(poly)
    ...
    def __pow__(self, k):
        ...
        return ((self * self) ** (k // 2)) * self
    
    def __ipow__(self, k):
        self.x = copy.deepcopy((self ** k).x)
        return self
